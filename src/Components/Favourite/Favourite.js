...
export default function Favourite() {
    ...
    searchReducer.formDate.map((item) => (
      <div>
        <ul className="favourite__block">
          ...
          <div>
            <button className="favourite__button favourite__button--change" onClick={()=>changeInfo(item.id)}>Изменить</button>
            <button
  ...
  return (
    <div>
      <h1>Избранное</h1>
      {showForm ? <Form setShowForm={setShowForm} changeValue={true} formInfo={formInfo}/> : null}
      {favouriteDate}
      {showFavouriteForm ? <FavouriteForm setShowFavouriteForm={setShowFavouriteForm} formInfo={formInfo} /> : null}
    </div>
  );
}
