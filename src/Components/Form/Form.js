...
export const Form = ({ setShowForm, formInfo, changeValue }) => {
  const [sortingType, setSortingType] = useState(
    formInfo === undefined ? "date" : formInfo[0].sortingType
  );
  const [formName, setFormName] = useState(
    formInfo === undefined ? null : formInfo[0].formName
  );
  ...
  const [request, setReques] = useState(
    formInfo === undefined ? searchReducer.searchFilm : formInfo[0].request
  );
  ...
  const setFormDate = (e) => {
    ...
    if (changeValue) {
      dispatch({
        type: "CHANGE_VALUE",
        value: { sortingType, request: request, formName, rangeValue, id },
      });
    } else {
      dispatch({
        type: "SET_DATA",
        value: {
          sortingType,
          request: searchReducer.searchFilm,
          formName,
          rangeValue,
          id: randomId,
        },
        ...
        <div>
          ...
          <label>
            ...
            <span className="form-validation">
              {formName && formName.length < 3
                ? "Название должно иметь больше 2х символов"
                : null}
            </span>
            <input
